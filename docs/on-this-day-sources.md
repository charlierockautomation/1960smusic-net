...
## IMAGES & VIDEO
- **Every single card gets a video. No exceptions, no "let the badge carry it."** If the event itself has no direct footage (a gig with no surviving recording, a flight, a birth, a tour opening), find the closest legitimately-connected official clip and make the connection explicit in the card copy: the song from the recording session that happened the day before/after, the single the artist had just released or was touring behind, a track the newborn's musician parent was recording that same week, etc. Weave the tie-in into summary_line_1 or summary_line_2 as a real sentence, not a bolted-on caption, so the embed reads as earned rather than filler.
- **Verify every ID via oEmbed before use**, same as always: fetch `https://www.youtube.com/oembed?url=...&format=json` and confirm `author_name` reads as an official source (VEVO, "{Artist} - Topic", the artist's own channel, a label channel). A title that says "Official Video" is not enough on its own, the search snippet can lie; always check the actual oEmbed `author_name`. Never use a fan reupload just because it's the only search hit, keep looking until an official one turns up.
- **Album/single cover art:** usable in an editorial/commentary context (same convention already used on ClassicRockArtists.com) for release-type events, and as a fallback only when no fitting video, official or contextual, can be found after a real search.
- **Artist photos:** only pull from Wikimedia Commons or another confirmed public-domain/CC-licensed source — do not pull arbitrary artist photos from a general image search, most 1960s press photography is still under copyright. Use only when video and cover art both come up empty.
